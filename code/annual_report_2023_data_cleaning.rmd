...
```{r}
data_ssari <- read_csv("raw_data/NovelCoronavirusCOVI_DATA_2024-05-20_1018.csv")
...
```{r}
data_ssari %>% group_by(subjid) # 12,990 patients
```
...
#                               variable=="bipap" ~ "daily_bipap_prtrt1",
#                               variable=="cpap" ~ "daily_cpap_prtrt1",
...
#   mutate(daily_dsstdat = as.character(daily_dsstdat),
#          daily_noninvasive_prtrt1 = daily_bipap_prtrt1,
#          daily_noninvasive_prtrt1 = ifelse(is.na(daily_noninvasive_prtrt1) & !is.na(daily_cpap_prtrt1), daily_cpap_prtrt1, daily_noninvasive_prtrt1))
# 
# final <- data_ssari %>%
#   left_join(test, by = c("subjid","daily_dsstdat")) %>%
#   mutate(daily_invasive_prtrt = ifelse(is.na(daily_invasive_prtrt) & daily_invasive_prtrt1==1, 1, daily_invasive_prtrt),
#          daily_ecmo_prtrt = ifelse(is.na(daily_ecmo_prtrt) & daily_ecmo_prtrt1==1, 1, daily_ecmo_prtrt),
#          daily_noninvasive_prtrt = ifelse(is.na(daily_noninvasive_prtrt) & daily_noninvasive_prtrt1==1, 1, daily_noninvasive_prtrt),
#          daily_rrt_cmtrt = ifelse(is.na(daily_rrt_cmtrt) & daily_rrt_cmtrt1==1, 1, daily_rrt_cmtrt),
#          daily_nasaloxy_cmtrt = ifelse(is.na(daily_nasaloxy_cmtrt) & daily_nasaloxy_cmtrt1==1, 1, daily_nasaloxy_cmtrt),
#          daily_prone_cmtrt = ifelse(is.na(daily_prone_cmtrt) & daily_prone_cmtrt1==1, 1, daily_prone_cmtrt),
#          daily_trach_prperf = ifelse(is.na(daily_trach_prperf) & daily_trach_prperf1==1, 1, daily_trach_prperf),
#          daily_inotrope_cmyn = ifelse(is.na(daily_inotrope_cmyn) & daily_inotrope_cmyn1==1, 1, daily_inotrope_cmyn),
#          daily_bipap = ifelse(is.na(daily_bipap) & daily_bipap_prtrt1==1, 1, daily_bipap),
#          daily_cpap = ifelse(is.na(daily_cpap) & daily_cpap_prtrt1==1, 1, daily_cpap),
#          daily_noninvasive_prtrt = ifelse((is.na(daily_noninvasive_prtrt) | daily_noninvasive_prtrt==0) & (daily_bipap==1 | daily_cpap==1), 1, daily_noninvasive_prtrt),
#          daily_noninvasive_prtrt = ifelse(is.na(daily_noninvasive_prtrt) & !is.na(daily_bipap), daily_bipap, daily_noninvasive_prtrt),
#          daily_noninvasive_prtrt = ifelse(is.na(daily_noninvasive_prtrt) & !is.na(daily_cpap), daily_cpap, daily_noninvasive_prtrt)) %>%
#   select(-c(daily_bipap_prtrt1:daily_noninvasive_prtrt1))
```
...
```{r}
saveRDS(data_ssari, "cleaned_data/sprit_sari_annual_report_2023_dataset.rds")
```
